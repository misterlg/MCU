...
void Pid_Loc_Init(void)
{
	...
	pid_loc.kp = (float)para_p;		
    pid_loc.ki = (float)para_p * 20 / (para_i*10);
    pid_loc.kd = (float)para_p / 20 * (para_d*10);
}
...
{    
	...
	if(pid_loc.integral > para_ar*10)
		pid_loc.integral = para_ar*10;
        ...
        case 5: if(pid_loc.actual >= pid_loc.set)
                {
                    ...
                    *kp_value = (float)para_p;		
                    *ki_value = (float)para_p * 20 / (para_i*10);
                    *kd_value = (float)para_p / 20 * (para_d*10);
